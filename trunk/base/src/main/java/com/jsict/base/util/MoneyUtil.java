...
public class MoneyUtil
{
     ...
     * @return
     */
    public static <E extends Number> String amountToChinese(E amount)
    {
        double dAmount = amount.doubleValue();
        if(dAmount > 99999999999999.99 || dAmount < -99999999999999.99)
        ...
        long temp = Math.round(dAmount * 100);
    ...
    public static <E extends Number> String thousandDot(E amount, int scale)
    {
        BigDecimal num = (new BigDecimal(amount.toString())).setScale(scale,
            Consts.ROUND_MODE);
        String format = "###,###";
        if(scale > 0)
        {
